source 'https://api.berkshelf.com'

metadata

# Resolve transitive deps of artifact-deployer
cookbook 'maven', git: 'https://github.com/Alfresco/maven.git', tag: 'v1.2.0-fork'
cookbook 'file', git: 'https://github.com/jenssegers/chef-file.git', tag: 'v1.0.0'
cookbook 'commons', git: 'https://github.com/Alfresco/chef-commons.git', tag: 'v0.5.7'
cookbook 'database', git: 'https://github.com/Alfresco/database.git'
cookbook 'alfresco-appserver', git: 'https://github.com/Alfresco/chef-alfresco-appserver.git'
# cookbook 'alfresco-appserver', path: '../chef-alfresco-splitting/chef-alfresco-appserver'
cookbook 'alfresco-utils', git: 'https://github.com/Alfresco/chef-alfresco-utils'
cookbook 'apache_tomcat', git: 'https://github.com/Alfresco/chef-apache_tomcat'
