...
##### Using Docker
```
KITCHEN_YAML=".kitchen.docker.yml" kitchen converge community
```
...
Access to the admin console is via (http://localhost/alfresco/)[http://localhost/alfresco] and then Alfresco Administration Console.
...
Check [Berksfile](https://github.com/Alfresco/chef-alfresco/blob/master/Berksfile) for more info; you can also use Librarian to resolve transitive dependencies.


Default Configurations
---

The most important configurations of chef-alfresco can be found in [attributes/default.rb](https://github.com/Alfresco/chef-alfresco/blob/master/attributes/default.rb); hereby the most important ones, as they define the components to use and the deployment features to enable:
...
Components
---
For each component, chef-alfresco may include external Chef cookbooks and/or change some attribute's defaults; the logic is implemented in the Chef-Alfresco [default recipe](https://github.com/Alfresco/chef-alfresco/blob/master/recipes/default.rb)

#### tomcat

Installs and configures Apache Tomcat using a [fork of Tomcat community cookbook](https://github.com/enzor/tomcat):
...
The complete list of configurable attributes can be found in [tomcat attributes](https://github.com/Alfresco/chef-alfresco/blob/master/recipe/_tomcat-attributes.rb); java configuration is included in the [default attribute file](https://github.com/Alfresco/chef-alfresco/blob/master/attributes/default.rb)
...
Fetch Alfresco WAR from a public/private Maven repository, URL or file-system (using [artifact-deployer](https://github.com/enzor/artifact-deployer)); by default, Chef Alfresco will fetch [Alfresco Repository 5.1.1 WAR](https://artifacts.alfresco.com/nexus/index.html#nexus-search;gav~org.alfresco~alfresco~5.1.1~war~), but you can override Maven coordinates to fetch your custom artifact (or define a url/path , check  [artifact-deployer docs](https://github.com/enzor/artifact-deployer)); since the WAR already includes log4j.properties and alfresco-global.properties, we need to disable the file generation features
...
```
You can optionally patch an existing share-config-custom.xml replacing all `@@key@@` (term delimiters are [configurable](https://github.com/enzor/artifact-deployer/blob/master/attributes/default.rb)) occurrences with attribute values of `node['alfresco']['shareproperties']` values; to enable this feature you must define the following attributes:
```
"alfresco": {
...
HAproxy is installed as OS package (using [haproxy community cookbook](https://github.com/hw-cookbooks/haproxy)) and configured using attributes defined in [haproxy.rb attribute file](https://github.com/Alfresco/chef-alfresco/blob/master/recipes/_haproxy-attributes.rb)
...
```
KITCHEN_YAML=.kitchen.docker.yml kitchen converge community
KITCHEN_YML=.kitchen.docker.yml kitchen verify community
```
...
Dependencies
---
Chef-Alfresco delegates the installation of 3rd party software to external cookbooks; you can find a complete list in [metadata.rb](https://github.com/alfresco/chef-alfresco/blob/master/metadata.rb)
