...
control 'alfresco-10' do
  ...
  describe file("#{ssl_folder}/#{filename}.key") do
    it { should exist }
    its('content') { should match "#{ssl_databag_test['key']}" }
  end

  describe file("#{ssl_folder}/#{filename}.crt") do
    it { should exist }
    its('content') { should match "#{ssl_databag_test['crt']}" }
  end

  describe file("#{ssl_folder}/#{filename}.chain") do
    it { should exist }
    its('content') { should match "#{ssl_databag_test['chain']}" }
  end

  describe file("#{ssl_folder}/#{filename}.nginxcrt") do
    it { should exist }
    its('content') { should match "#{ssl_databag_test['nginxcrt']}" }
  end

  describe file("#{ssl_folder}/#{filename}.dhparam") do
    it { should exist }
    its('content') { should match "#{ssl_databag_test['dhparam']}" }
  end
end
